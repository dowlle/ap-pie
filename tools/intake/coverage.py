...
def export(db, expected_prs=()):
    ...
    for candidate in candidates:
        ...
        state = 'verified' if any(r['verified'] >= candidate['discovered'] for r in exact) else candidate['state']
    ...
    for number in sorted(pr_numbers, key=int):
        ...
        pr_report.append({'number': int(number), 'candidate_states': states,
                          ...
                          'artifact_verification_pending' if any(i['state'] != 'verified' for i in items) else None})
    held = []
