The Adventure Mode warp pads no longer have to follow the original game's unlock order. A generated world can give each physical pad a new requirement, and the display above the pad tells you what that seed expects.

## Read the symbols above the pad

The floating symbol is the requirement for that physical pad. It can ask for a number of Trophies, Keys, Relics, CTR Tokens, or Gems. A count next to the symbol tells you how many are needed.

These are Archipelago items. You can receive them from checks in CTR or from another game in the same multiworld. Finishing more races is therefore not always the immediate answer. Someone else may be holding the item that opens your route. [Progression and kart upgrades](/ctr/reference/progression) explains which items open the Adventure hubs.

A pad that is free has no item requirement. A requirement that you already satisfy is treated as open rather than falling back to the original game's requirement.

## Physical pad and destination are separate
...
After you clear the Trophy Race, those second-phase races become available when that requirement is met. You do not need to break every [AP box](/ctr/reference/ap-boxes) first. If the pad has no second requirement, clearing the Trophy Race is enough.
...
With [racer-locked pads](/ctr/reference/randomized-content#characters), the display also names the required character. `REQUIRES <CHARACTER>` means that racer has not been received yet; the manual character selector uses `NOT UNLOCKED`. Receiving the racer allows the pad to load its race with that character, subject to the other requirements.
