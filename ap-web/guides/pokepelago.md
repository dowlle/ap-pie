...
This guide reflects the current published release, Poképelago APWorld **0.6.5**. That APWorld requires **Archipelago 0.6.7 or newer**. In an organized multiworld, always use the exact APWorld version chosen by the host.
...
- [`pokepelago.apworld` for version 0.6.5](https://github.com/dowlle/PokepelagoClient/releases/download/v0.6.5/pokepelago.apworld);
- [`Pokepelago.yaml` from the same release](https://github.com/dowlle/PokepelagoClient/releases/download/v0.6.5/Pokepelago.yaml), or a YAML created for that same APWorld version;
...
- Version 0.6.5 will not randomly select Hisui as the only region. Hisui has only seven Pokémon, which is too small for some heavy lock combinations.

You can still request manual Hisui-only play with a light lock setup. Version 0.6.5 rejects a manual solo-Hisui configuration when two or more lock systems are enabled, before generation reaches a cryptic `FillError`. Pair Hisui with Galar or another region, or disable some locks.
...
Version 0.6.5 does not randomly choose Hisui as the only region. If you manually select only Hisui and enable two or more lock systems, generation stops with an option error explaining the constraint. Add Galar or another region, or disable some locks.
