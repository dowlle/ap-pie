...
export default function APWorldEvidence({ version, builtin = false }: { version?: APWorldVersion; builtin?: boolean }) {
  ...
  const builtinRelease = version ? version.source === "builtin" : builtin;
  ...
  return <>
    <div className="apworld-evidence-pair">
      <EvidenceButton label={reviewLabel} tone={reviewTone} explanation={reviewExplanation} expanded={detail === "review"} onClick={() => setDetail(detail === "review" ? null : "review")} />
    ...
    {detail === "review" && <div className="apworld-evidence-detail" role="region" aria-label="Security review explanation"><strong>Security review{version ? ` · v${version.version}` : ""}</strong><p>{reviewExplanation}</p>
      {review && <>
      ...
      <button type="button" className="btn btn-quiet btn-sm" onClick={() => setDetail(null)}>Close explanation</button></div>}
